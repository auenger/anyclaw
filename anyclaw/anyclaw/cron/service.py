...
class CronService:
            ...
            try:
                ...
                for j in data.get("jobs", []):
                    ...
                    jobs.append(CronJob(
                        ...
                        payload=CronPayload(
                            kind=j.get("payload", {}).get("kind", "agent_turn"),
                            message=j.get("payload", {}).get("message", ""),
                            deliver=j.get("payload", {}).get("deliver", False),
                            channel=j.get("payload", {}).get("channel"),
    ...
    async def start(self) -> None:
        ...
        self._save_store()
        self._arm_timer()
        logger.info(f"Cron service started with {len(self._store.jobs) if self._store else 0} jobs")
