      ...
      <div className="p-4 border-b border-[var(--subtle-border)] space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-semibold">{t.logs.systemLogs || 'System Logs'}</h1>
            {isToday && (
              <button
                onClick={toggleLive}
                className={cn(
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative">
            ...
            {showDatePicker && (
              <div className="absolute top-full left-0 mt-1 z-10 rounded-lg border border-[var(--subtle-border)] bg-background p-2 shadow-lg">
                {/* Available dates */}
                <div className="max-h-60 overflow-y-auto space-y-1">
                  {availableDates.map((date) => (
                    <button
                      ...
                      className={cn(
                        "w-full text-left px-3 py-1.5 rounded text-sm hover:bg-accent",
                        date === selectedDate && "bg-accent font-medium"
                      )}
                    >
                      {date === new Date().toISOString().split('T')[0]
                        ? `${date} (${t.logs.today || 'Today'})`
                        : date}
                    </button>
                  ))}
                </div>
                {/* Custom date input */}
                <div className="mt-2 pt-2 border-t border-[var(--subtle-border)]">
                  <input
                    type="date"
                    value={selectedDate}
                    onChange={(e) => {
                    ...
                    className="w-full px-2 py-1 rounded border border-[var(--subtle-border)] bg-background text-sm"
                  />
                </div>
              </div>
            )}
      ...
      {stats && (
        <div className="p-2 border-t border-[var(--subtle-border)] text-xs text-muted-foreground bg-muted/30">
          <div className="flex justify-between">
            <span>{t.logs.systemLogs || 'System logs'}: {stats.system_logs_total}</span>
            <span>
              {t.logs.byLevel || 'By level'}: {Object.entries(stats.by_level).map(([k, v]) => `${k}:${v}`).join(' ')}
            </span>
          </div>
        </div>
      )}
